...
public class GameLogic {
    ...
    public boolean isHandPlayable(int playerIndex) {
    	ArrayList<BaseCard> CurrentHand = getPlayerHand(playerIndex);
    	boolean playable = false;
    	for(int i=0; i<CurrentHand.size(); i++)
    		playable |= CurrentHand.get(i).canPlay();
    	return playable;
    }

    public ArrayList<BaseCard> draw(int amount) {
        ArrayList<BaseCard> DrewCard = new ArrayList<BaseCard>();
        amount = Math.min(amount, getDeck().size());
        for(int i=0; i<amount; i++)
        	DrewCard.add(getDeck().remove(0));
        getCurrentPlayerHand().addAll(DrewCard);
        return DrewCard;
    }
