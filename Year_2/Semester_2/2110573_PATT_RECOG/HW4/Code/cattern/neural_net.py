...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # input - fully connected layer - ReLU - fully connected layer - softmax
    fc1_out = X @ W1 + b1
    relu_out = np.maximum(fc1_out, 0)
    scores = relu_out @ W2 + b2 # fc2_out
    ...
    P = exp_score / exp_score.sum(axis=1, keepdims=True)
    ...
    def fc_backward(dout, x, W):
      db = dout.sum(axis=0) # dout/dz * dz/db 
      dx = dout @ W.T
      dw = x.T @ dout
      return dx, dw, db
    ...
    P[range(N), y] -= 1
    dloss = P / N
    dloss_fc2, dw2, db2 = fc_backward(dloss, relu_out, W2)
    dloss_relu = relu_backward(dloss_fc2, fc1_out)
    dloss_fc1, dw1, db1 = fc_backward(dloss_relu, X, W1)
