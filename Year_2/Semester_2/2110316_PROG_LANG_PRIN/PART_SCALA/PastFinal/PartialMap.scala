...
object PartialMap {
  ...
  @tailrec
  def partialMap(l:List[String], acc:List[String]=Nil)(f1:String => String)(f2: String => Boolean): List[String] = {
    if(l.isEmpty) return acc
    val cur = l.head
    if( f2(cur) ) partialMap(l.tail, acc ++ List(f1(cur)))(f1)(f2)
    else partialMap(l.tail, acc ++ List(cur))(f1)(f2)
